    ...
    schema['properties'].transform_values{ |spec|
      ...
      filter_id = (
        if [%w[yes no], %w[no yes]].include?(enum)
          conn.exec(
            'INSERT INTO filters(project_id, type, field_id) VALUES ($1, $2, $3) RETURNING id',
  ...
  task :new, [] => :environment do
    PG.connect(host: 'postgres', dbname: 'postgres', user: 'postgres', password: 'postgres').transaction { |conn|
      ...
      if datasources_slug.nil?
        ...
        schema = fetch_json("#{datasource_url}/data/#{datasources_slug}/tags_schema.json")
        begin
          native_schema = fetch_json("#{url_base}/data/#{datasource_project}/natives_schema.json")
        rescue StandardError
          native_schema = nil
        end
        schema = schema.merge(native_schema) if native_schema.present?
