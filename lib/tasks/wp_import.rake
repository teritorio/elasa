...
def load_project(project_slug, datasources_slug, url)
  ...
  PG.connect(host: 'postgres', dbname: 'postgres', user: 'postgres', password: 'postgres') { |conn|
    ...
    project_id = conn.exec('SELECT id FROM projects WHERE slug = $1', [project_slug]) { |result|
      result.first['id'].to_i
    }
...
namespace :wp do
  ...
  task :import, [] => :environment do
    ...
    PG.connect(host: 'postgres', dbname: 'postgres', user: 'postgres', password: 'postgres').transaction { |_conn|
      ...
      load_menu(project_slug, project_id, nil, user_uuid, "#{base_url}/menu.json", "#{base_url}/pois.json", "#{base_url}/menu_sources.json", nil, policy_uuid, nil)
