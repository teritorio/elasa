...
def load_source(conn, project_slug, metadatas)
  ...
  conn.copy_data('COPY sources_import_raw FROM STDIN (FORMAT binary)', enco) {
    metadatas.each{ |id, metadata|
      conn.put_copy_data([
        ...
        metadata['report_issue_url_template'],
      ])
    }
  }
  ...
  conn.exec_params("
    MERGE INTO
      sources
    USING
      (SELECT DISTINCT project_id, slug, attribution, report_issue_url_template FROM sources_import) AS sources_import
    ON
    ...
    WHEN MATCHED THEN
      UPDATE SET
        ...
        report_issue_url_template = sources_import.report_issue_url_template
    WHEN NOT MATCHED THEN
      INSERT (project_id, slug, attribution, report_issue_url_template)
      VALUES (
        sources_import.project_id,
        sources_import.slug,
        sources_import.attribution,
        sources_import.report_issue_url_template
      )
  ")
