...
cat docker/directus/schema.sql docker/directus/data.sql docker/directus/seq.sql lib/locale-table.sql lib/api-01.sql lib/api-02.sql | docker compose exec -T -u postgres postgres psql -v ON_ERROR_STOP=1
```
...
```
docker compose run --rm script bundle exec rake api:diff -- https://tourism-seignanx.elasa.teritorio.xyz https://tourism-seignanx.elasa-dev.teritorio.xyz seignanx tourism
```
...
docker compose run --rm script bundle exec rails db:migrate
cat lib/locale-table.sql lib/api-01.sql lib/api-02.sql lib/api-02-cache.sql | docker compose exec -T -u postgres postgres psql -v ON_ERROR_STOP=1 -v project_id=NULL
...
```
docker compose run --rm script bundle exec rake api:validate -- http://api:12000/api/0.1/seignanx/tourism
docker compose run --rm script bundle exec rake api:validate_poi -- [project_slug] [theme_slug]
docker compose run --rm script bundle exec rake api:diff -- https://elasa.teritorio.xyz http://api:12000 [project_slug] [theme_slug]
```
